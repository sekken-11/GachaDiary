      ...
      <v-row>
        <v-col cols="12" md="8">
          <v-card class="bg-white shadow" id="top-form">
            <v-card-text class="text-center my-3 text-muted">
              <p>
                あなたの所持するガチャ石の数・ガチャ石の価格を入力してください
              </p>
            ...
            <Form @submit="handleConversion">
              <v-card-text class="m-3">
                <div class="form-group mb-5">
                  <label for="posses_stone" class="form-label">ガチャ石の個数</label>
                  <Field
                    name="posses_stone"
                    id="posses_stone"
                    v-model.number="posses_stone"
                    class="form-control"
                    type="text"
                    placeholder="現金換算したい石の個数を入力してください（個）"
                    :rules="isNumericRequired"
                  />
                  <div class="text-danger">
                    <ErrorMessage name="posses_stone" id="posses_stone_error" />
                  </div>
                </div>
                <div class="form-group my-5">
                  ...
                  />
                  <div class="text-danger">
                    <ErrorMessage name="price" id="stone_price_error" />
                  </div>
                  <label for="stone_quantity"></label>
                  <Field
                    name="quantity"
                    id="stone_quantity"
                    v-model.number="stone_quantity"
                    class="form-control"
                    type="text"
                  ...
                  />
                  <div class="text-danger">
                    <ErrorMessage name="quantity" id="stone_quantity_error" />
                  </div>
                </div>
                <div class="form-group mb-5">
                  <label for="game" class="form-label">ゲーム名*任意</label>
                  <input
                    id="game"
                    v-model="game_name"
                    class="form-control"
                ...
                <div class="form-group">
                  <label for="game_select" class="form-label">ゲーム選択</label>
                  <select v-model="select" class="form-control" id="select">
                    <option value="" selected>ゲームを選択しない</option>
                    <option
                      v-for="(initialPackage, index) in initialPackages"
        ...
        <v-col cols="12" md="4">
          ...
          <div class="amount rounded shadow p-3">
            ...
            <div
              v-for="(conversion_record, index) in conversion_records"
              :id="'record-' + (index + 1)"
              :key="conversion_record"
              class="bg-white border shadow-sm rounded my-2"
            >
              <v-container>
...
export default {
  ...
  methods: {
    ...
    handleConversion() {
      this.cash_conversion = Math.round(
        (this.stone_price / this.stone_quantity) * this.posses_stone
      );
      this.conversion_records_sub.push({
        name: this.game_name,
        price: this.cash_conversion,
        posses_stone: this.posses_stone,
      });
